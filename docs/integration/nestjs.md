...
Use guards for more complex validation scenarios or when you need to validate before the route handler executes.
...
```typescript
@Get()
@UseGuards(new ValidationGuard({ schema: querySchema, source: 'query' }))
getUsers(@Req() req: any) {
  // Access validated data from req.validatedQuery
...
```typescript
@Get(':id')
@UseGuards(new ValidationGuard({ schema: paramSchema, source: 'params' }))
getUser(@Req() req: any) {
  // Access validated data from req.validatedParams
  const params = req.validatedParams;
  return { userId: params.id };
}
```

### Legacy Guard Syntax (Still Supported)
...
Use the `ValidationExceptionFilter` to format validation errors consistently across your application.
...
Apply the filter globally in your `main.ts`:
...
You can also validate data in your service layer using the `validate` function.
...
export class UsersService {
  async createUser(data: unknown) {
    ...
    return this.saveUser(result.data);
  }
...
- `ValidationExceptionFilter` - Formats validation errors consistently
...
- `validate(schema, data)` - Validate data in service layer
